...
<body>
            ...
            <ul class="fixCen pr rs">
                ...
                @foreach (\App\Site::getAllParentCategories() as $parentCategory)
                    @if ($parentCategory->subCategories->count() > 0)
                        <li class="parentMenu">
                            <a href="{{url( $parentCategory->slug)}}" title="{{$parentCategory->name}}" class="{{(isset($page) && $page == $parentCategory->slug) ? 'active' : ''}}">{{$parentCategory->name}}</a>
                            <ul class="submenu">
                                @foreach ($parentCategory->subCategories as $subCategory)
                                    <li><a href="{{url( $subCategory->slug)}}" title="{{$subCategory->name}}">{{$subCategory->name}}</a></li>
                                @endforeach
                            </ul>
                        </li>
                    @else
                        <li><a href="{{url( $parentCategory->slug)}}" title="{{$parentCategory->name}}" class="{{(isset($page) && $page == $parentCategory->slug) ? 'active' : ''}}">{{$parentCategory->name}}</a></li>
                    @endif
                @endforeach

                <li><a class="{{(isset($page) && $page == 'cau-hoi-thuong-gap') ? 'active' : ''}}" href="{{url('cau-hoi-thuong-gap')}}" title="Hỏi đáp">Hỏi đáp</a></li>
        ...
        <div class="block8">
            <div class="fixCen">
                @foreach (\App\Site::getLatestQuestions(6) as $question)
                    <div class="share">
                    <h4 class="rs title"><a href="{{url('cau-hoi-thuong-gap', $question->slug)}}"title="Câu hỏi" target="_blank">{{$question->title}}</a></h4>
                    <div class="ques-detail">
                       {!! $question->question !!}
                    </div>
                    <div class="view-more"><a href="{{url('cau-hoi-thuong-gap', $question->slug)}}"title="Xem trả lời">Xem trả lời</a></div>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="others">
        <div class="fixCen">
            <div class="content">
                <div class="links">
                    <a href="{{url( 'san-pham')}}" title="Thông tin sản phẩm">Thông tin sản phẩm</a>
                    <a href="{{url( 'nghien-cuu-lam-sang')}}" title="Nghiên cứu khoa học">Nghiên cứu khoa học</a>
            ...
            <div class="content">
                <div class="links">
                    ...
                    <a href="{{url( 'men-gan-cao')}}" title="Men gan cao">Men gan cao</a>
                </div>
            </div>
            <div class="content">
