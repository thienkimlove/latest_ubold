...
<div class="{{($key_type == 'index_block_1') ? 'block-2 pr' : 'block-2 block-3 pr'}}">
    ...
    <div class="tab-content">
        @if ($posts = \App\Lib\Helpers::getIndexCategoryPosts($category))
            <div id="tab-{{($key_type == 'index_block_1')? '1' : '5'}}" class="content active">
                    ...
                    <div class="hot-news">
                        <div class="post">
                            <a href="{{url($firstPost->slug.'.html')}}" class="title">
                                <img src="{{url('files', $firstPost->image)}}" alt="" width="301" height="183"></a>
                ...
                <div class="news">
                    @foreach ($posts as $post)
                        <div class="post">
                            <a href="{{url($post->slug.'.html')}}" class="thumbs" style="background-image: url({{url('files', $post->image)}})">
                                <img src="{{url('files', $post->image)}}" alt="" width="126" height="90">
                            </a>
                            <div class="news-info">
        ...
        @endif
        @foreach ($subCategories as $v => $subCategory)
            @if ($posts = \App\Lib\Helpers::getIndexCategoryPosts($subCategory))
                <div id="tab-{{($key_type == 'index_block_1')? $v+2 : $v+6}}" class="content">
                    @if ($firstPost = $posts->shift())
                        <div class="hot-news">
                            <div class="post">
                                <img src="{{url('files', $firstPost->image)}}"
